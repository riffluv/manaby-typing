...
export function useOptimizedTyping(options = {}) {
  ...
  const updateScore = useCallback((data = {}) => {
    ...
    setScoreData((prevScore) => {
      ...
      if (isCorrect) {
        ...
        const pointsEarned =
          Math.ceil(baseScore + cpmScore + comboScore) * scoreFactor;
  ...
  const calculateRank = useCallback((score, baseScore) => {
    ...
    if (score >= baseScore * 1.1) return 'SS';
    if (score >= baseScore * 1.05) return 'S+';
    if (score >= baseScore * 1.0) return 'S';
    if (score >= baseScore * 0.95) return 'A+';
    if (score >= baseScore * 0.9) return 'A';
    if (score >= baseScore * 0.85) return 'B+';
    if (score >= baseScore * 0.8) return 'B';
    if (score >= baseScore * 0.75) return 'C+';
    if (score >= baseScore * 0.7) return 'C';
    if (score >= baseScore * 0.6) return 'D+';
    if (score >= baseScore * 0.5) return 'D';
    if (score >= baseScore * 0.4) return 'E+';
    if (score >= baseScore * 0.3) return 'E';
    if (score >= baseScore * 0.2) return 'F+';
    return 'F';
  }, []);
  ...
  const handleLineEnd = useCallback(
    ({ leftover = 0, completed = false, combo = 0, maxCombo = 0 }) => {
      // スキップされた文字数をスコアに反映
      updateScore({
        timestamp: Date.now(),
        isSkipped: true,
   ...
   * 正解キー入力のカウント
   */
  const countCorrectKey = useCallback(
    (timestamp = Date.now()) => {
      const stats = statsRef.current;
