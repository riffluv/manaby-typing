...
export default class TypingUtils {
  ...
  static convertFullWidthToHalfWidth(char) {
    ...
    // マッピングテーブルに存在する場合は変換、そうでなければそのまま返す
    return fullToHalfMap[char] || char;
  }

  /**
   * IMEの状態を検出する関数（英数モードかどうか）
   * @return {boolean} 英数モードならtrue、日本語入力モードならfalse
   */
  static detectAlphanumericMode() {
    // モダンブラウザでのIME状態検出（可能な場合）
    if (window.InputMethodManager && window.InputMethodManager.isAlphanumeric) {
      return window.InputMethodManager.isAlphanumeric();
    }
    
    // 直接検出できない場合は、最後に入力された文字のタイプで推測
    return true; // デフォルトでは英数モードと仮定
  }

  /**
   * IME入力の処理
   * @param {Event} event - 入力イベント
   * @return {string|null} 変換されたテキスト、またはnull
   */
